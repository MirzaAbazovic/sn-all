...
import de.bitconex.tmf.party.configuration.ApplicationContextProvider;
...
public abstract class ModelUtils {
    ...
    public static void setIdAndHref(Object object, String path) {
        try {
            ...
            if (modelId.get(object) != null)
                objId = modelId.get(object).toString();
            else
                modelId.set(object, objId);

            Environment env = ApplicationContextProvider.getContext().getEnvironment();

            String href = env.getProperty("api.host") + env.getProperty("server.contextPath") + path + "/" + objId;
