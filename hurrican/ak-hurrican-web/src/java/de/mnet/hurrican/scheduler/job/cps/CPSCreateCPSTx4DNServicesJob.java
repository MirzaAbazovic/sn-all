...
/**
 * Job-Implementierung, um CPS-Transactions fuer Aenderungen an Rufnummernleistungen von aktiven Auftraegen zu
 * erstellen. <br> Die gesamte Logik ist in den Service-Klassen enthalten. Die Job-Klasse uebernimmt noch gewisse
 * Logging- und Messaging-Funktionen.
 *
 *
 */
public class CPSCreateCPSTx4DNServicesJob extends AbstractCPSJob {

    private static final Logger LOGGER = Logger.getLogger(CPSCreateCPSTx4DNServicesJob.class);

    private static final String RESOURCE = "de.mnet.hurrican.scheduler.job.cps.resources.CPSCreateCPSTx4DNServicesJob";
    ...
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        ...
        try {
            ...
            // CPS-Tx erstellen
            CPSTransactionResult result = cps.createCPSTransactions4DNServices(actDate, HurricanScheduler.getSessionId());
            ...
            if ((cpsTxCount > 0) || warnings.isNotEmpty()) {
                final String warningsAsText = ObjectTools.getOrElse(warnings.getWarningsAsText(), "none");
                ...
                sendMail(
                        ...
                        StringTools.formatString(
                                ...
                                new Object[] { DateTools.formatDate(actDate, DateTools.PATTERN_DAY_MONTH_YEAR),
            ...
            sendMail(mailConfig.getMailTo(), HurricanConstants.EMAIL_SEPARATOR, mailConfig.getMailSubject(),
                    StringTools.formatString(
                            ...
                            new Object[] { DateTools.formatDate(actDate, DateTools.PATTERN_DAY_MONTH_YEAR),
                                    DateTools.formatDate(new Date(), DateTools.PATTERN_DATE_TIME_LONG), e.getMessage() }
                    ),
                    context
            );
        }
    ...
    private void initJob() throws Exception {
        try {
            ...
            LOGGER.error(e.getMessage(), e);
            throw new AKSchedulerException("Error initializing the Job 'CPSCreateCPSTx4DNServicesJob': " + e.getMessage());
        }
    }
