...
public class ProcessWbciAutomatableAkmTRsJobTest {
    @Mock
    ...
    @Spy
    @InjectMocks
    private ProcessWbciAutomatableAkmTRsJob job = new ProcessWbciAutomatableAkmTRsJob() {
        @Override
    ...
    public void testExecuteInternal() throws Exception {
        ...
        when(wbciAutomationServiceMock.processAutomatableAkmTrs(user)).thenReturn(Collections.<String>emptyList());
        doReturn(user).when(job).getCurrentUser(anyLong());

        job.executeInternal(jobExecutionContextMock);

        verify(wbciAutomationServiceMock).processAutomatableAkmTrs(user);
    ...
    @Test
    public void testExecuteInternalAkmTrWithRuntimeException() throws Exception {
        RuntimeException runtimeException = new RuntimeException();
        AKUser user = new AKUser();
        doThrow(runtimeException).when(wbciAutomationServiceMock).processAutomatableAkmTrs(user);
        doReturn(user).when(job).getCurrentUser(anyLong());
