...
public class ProcessWbciAutomatableErlmTvsJobTest {
    ...
    @Spy
    @InjectMocks
    private ProcessWbciAutomatableErlmTvsJob job = new ProcessWbciAutomatableErlmTvsJob() {
        @Override
    ...
    public void testExecuteInternal() throws Exception {
        ...
        when(wbciAutomationServiceMock.processAutomatableErlmTvs(any(AKUser.class))).thenReturn(Collections.<String>emptyList());
        doReturn(user).when(job).getCurrentUser(anyLong());

        job.executeInternal(jobExecutionContextMock);

        verify(wbciAutomationServiceMock).processAutomatableErlmTvs(user);
    ...
    public void testExecuteInternalWithRuntimeException() throws Exception {
        ...
        doThrow(runtimeException).when(wbciAutomationServiceMock).processAutomatableErlmTvs(user);
