...
IS  
   BEGIN
    FOR uevtid IN (select UEVT.UEVT_ID from T_UEVT uevt 
      ...
      insert into T_RANGIERUNGSMATRIX matrix 
        ...
        null, null, null, to_date('13/03/2012','DD/MM/YYYY'), to_date('01/01/2200','DD/MM/YYYY'), 'Trenker')
      ;
    END LOOP;
   END;
/
...
-- Wegwerfen
drop procedure rangmatrix_wholesale_fttb;

-- Fehlendes Constraint ergaenzen
alter table T_HVT_STANDORT add constraint fk_hvtstd_stype_2_ref 
foreign key (STANDORT_TYP_REF_ID) references T_REFERENCE(ID)
;