...
VALUES
  (17, 'Online Gutschrift Händler',
   ...
   'BILLING',
   'select jet_user.USERNAME as BEARBEITER, CUSTOMER.CUST_NO as HAENDLER_NO, CUSTOMER.NAME as HAENDLER_NAME, CUSTOMER.FIRSTNAME as HAENDLER_VORNAME,
  ...
  left join JET_USER jet_user on AUFTRAG.BEARBEITER_NO=jet_user.PERSON_NO
  ...
  and AUFTRAG.OE__NO not in (3000, 3090)
  ...
  and AUFTRAG.AUFTRAG__NO not in (
    select AUFTRAGTMP.AUFTRAG__NO
      ...
      where
        ...
        and (
          LEISTUNG.NAME=''G/Online-Bestellung Händler''
          or LEISTUNG.NAME=''G/Umzug Händler''
          or LEISTUNG.NAME=''G/Vertragsverlängerung Händler''
          )
  )',
   'von=date,bis=date', 0);
