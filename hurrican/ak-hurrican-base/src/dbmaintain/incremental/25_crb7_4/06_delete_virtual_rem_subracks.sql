...
-- KEN-DIESE-018-MFG Subrack 1-1, Subracktyp NFXS-B -> 8*48=384
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-HEUST-047-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);

  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-KIESE-059-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-KIESE-003-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-NIEDE-066-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-ELIAU-006b-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'ERL-BRUCK-008-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'KEN-DIESE-000-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
  ...
  delete from T_HW_SUBRACK sr
    where sr.RACK_ID in
    (select r.ID from T_HVT_GRUPPE g, T_HVT_STANDORT s, T_HW_RACK r
      where g.ORTSTEIL = 'KEN-DIESE-018-MFG'
      and g.HVT_GRUPPE_ID = s.HVT_GRUPPE_ID
      and s.HVT_ID_STANDORT = r.HVT_ID_STANDORT)
    and sr.ID not in
    (select b.SUBRACK_ID from T_HW_BAUGRUPPE b where b.RACK_ID=sr.RACK_ID);
