...
for each row
  begin
    if (:NEW.RANG_SCHNITTSTELLE is not null and :NEW.RANG_SCHNITTSTELLE not in ('LWL', 'FTTB_LZV', 'H', 'N', 'CFV', 'FTTB_MNET', 'N', 'FTTB_LVT', 'Fttb'))
    then
      raise_application_error(-20000, 'RANG_SCHNITTSTELLE is not valid');
    end if;
    
    if (:NEW.STATUS is not null and :NEW.STATUS not in ('abgebaut', 'locked', 'res', 'vorb', 'defekt', 'frei', 'rang'))
    then
      raise_application_error(-20000, 'STATUS is not valid');
    end if;
    
    if (:NEW.UETV is not null and :NEW.UETV not in ('H02', 'H03', 'H04', 'H05', 'H07', 'H08', 'H11', 'H13', 'H16', 'H18', 'LWL', 'N01'))
    then
