...
/**
 * {@link DisableEinwahlaccountCommand} Unit Test
 */
@Test(groups = BaseTest.UNIT)
public class DisableEinwahlaccountCommandTest extends BaseTest {

    private CCLeistungsService ccLeistungsServiceMock;
    private AccountService accountServiceMock;

    private DisableEinwahlaccountCommand cut;

    @BeforeMethod
    public void setUp() throws FindException {
        cut = new DisableEinwahlaccountCommand();
    ...
    public void toggleAccount_WithoutAccount() throws Exception {
        when(accountServiceMock.findIntAccounts4Auftrag(any(Long.class), eq(IntAccount.LINR_EINWAHLACCOUNT)))
                .thenReturn(null);
        cut.disableAccount(true);
        verify(accountServiceMock, times(0)).saveIntAccount(any(IntAccount.class), eq(true));
    }

    @Test
    public void toggleAccount_accountAlreadyLocked() throws Exception {
    ...
    @Test
    public void toggleAccount_LockAccount() throws Exception {
        IntAccount account = new IntAccount();
        ...
        assertEquals(account.getGesperrt(), Boolean.TRUE);
    }
}
