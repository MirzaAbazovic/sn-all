...
public class CBVorgangNoReturnOnOpenCbvTest extends BaseTest {
    ...
    public void test(Long state, Boolean returnOk, boolean expected) {
        CBVorgang cbVorgang = new CBVorgangBuilder().withStatus(state).withReturnOk(returnOk).setPersist(false).build();

        assertThat(new CBVorgangNoReturnOnOpenCbvValidator().isValid(cbVorgang, context), equalTo(expected));
