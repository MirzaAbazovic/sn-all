...
import de.mnet.wita.model.builder.cdm.lineordernotificationservice.v1.AbmJaxbBuilder;
import de.mnet.wita.unmarshal.v1.AbmUnmarshaller;
...
public class AbmUnmarshallerTest extends BaseTest {
    ...
    private final AbmUnmarshaller abmReader = new AbmUnmarshaller();
            ...
            for (MeldungsPositionWithAnsprechpartner witaMeldungsPosition : meldungsPositionen) {
                if (witaMeldungsPosition.getMeldungsCode().equals(xmlMeldungsPosition.getMeldungscode())) {
                    ...
                    assertThat(ansprechpartnerTelekom.getAnrede(),
                            equalTo(AnredeConverter.toMwf(abmBuilder.ansprechpartnerTelekom.getAnrede())));
    ...
    public void rufnummerPortierungShouldBeRead() {
        AbmJaxbBuilder abmBuilder = new AbmJaxbBuilder().addRufnummernportierung("0821", "123456");
        AuftragsBestaetigungsMeldung unmarshalled = (AuftragsBestaetigungsMeldung) abmReader.unmarshal(abmBuilder.build());
        assertThat(unmarshalled, notNullValue());
        assertThat(unmarshalled.getRufnummernPortierung(), notNullValue());

        RufnummernPortierungEinzelanschluss portierung = (RufnummernPortierungEinzelanschluss) unmarshalled.getRufnummernPortierung();
        assertThat(portierung.getRufnummern().get(0).getOnkz(), equalTo("0821"));
        assertThat(portierung.getRufnummern().get(0).getRufnummer(), equalTo("123456"));
    }


    public void rufnummerPortierungBlockShouldBeRead() {
        AbmJaxbBuilder abmBuilder = new AbmJaxbBuilder().addRufnummernportierung("0821", "123456", "0", "00", "99");
        AuftragsBestaetigungsMeldung unmarshalled = (AuftragsBestaetigungsMeldung) abmReader.unmarshal(abmBuilder.build());
        assertThat(unmarshalled, notNullValue());
        assertThat(unmarshalled.getRufnummernPortierung(), notNullValue());

        RufnummernPortierungAnlagenanschluss portierung = (RufnummernPortierungAnlagenanschluss) unmarshalled.getRufnummernPortierung();
        assertThat(portierung.getOnkz(), equalTo("0821"));
        assertThat(portierung.getAbfragestelle(), equalTo("0"));
        assertThat(portierung.getDurchwahl(), equalTo("123456"));
        assertThat(portierung.getRufnummernBloecke(), hasSize(1));

        RufnummernBlock block = portierung.getRufnummernBloecke().iterator().next();
        assertThat(block.getVon(), equalTo("00"));
        assertThat(block.getBis(), equalTo("99"));
    }
