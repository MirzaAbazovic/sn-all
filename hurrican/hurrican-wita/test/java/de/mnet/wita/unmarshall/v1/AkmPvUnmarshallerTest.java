...
import de.mnet.wita.model.builder.cdm.lineordernotificationservice.v1.AkmPvJaxbBuilder;
import de.mnet.wita.unmarshal.v1.AkmPvUnmarshaller;

@Test(groups = UNIT)
public class AkmPvUnmarshallerTest extends BaseTest {

    private final AkmPvUnmarshaller underTest = new AkmPvUnmarshaller();
    ...
    public void testEinzelanschlussPortierungShouldBeRead() {
        AnkuendigungsMeldungPv unmarshalled = unmarshal(new AkmPvJaxbBuilder()
                .withRufnummernPortierung(new RufnummernPortierungBuilder().buildAuftragPortierung(true)));
        assertThat(unmarshalled.getRufnummernPortierung(), notNullValue());
    }

    public void testAnlagenanschlussPortierungShouldBeRead() {
        AnkuendigungsMeldungPv unmarshalled = unmarshal(new AkmPvJaxbBuilder()
                .withRufnummernPortierung(new RufnummernPortierungBuilder().buildAuftragPortierung(false)));
        assertThat(unmarshalled.getRufnummernPortierung(), notNullValue());
    }
