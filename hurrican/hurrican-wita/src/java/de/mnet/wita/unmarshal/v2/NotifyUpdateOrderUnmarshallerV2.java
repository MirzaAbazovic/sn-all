...
@Component
public class NotifyUpdateOrderUnmarshallerV2 implements Function<NotifyUpdateOrder, WitaMessage> {

    private Map<Class<? extends MeldungstypAbstractType>, AbstractUnmarshallerFunctionV2<? extends MeldungstypAbstractType>> mappings;

    @Autowired
    private List<AbstractUnmarshallerFunctionV2<? extends MeldungstypAbstractType>> unmarshallerFunctions;

    @PostConstruct
    public void init() {
        mappings = new HashMap<>();
        for (AbstractUnmarshallerFunctionV2<? extends MeldungstypAbstractType> unmarshallerFunction : unmarshallerFunctions) {
    ...
    public WitaMessage apply(@Nullable NotifyUpdateOrder input) {
        ...
        meldung.setCdmVersion(WitaCdmVersion.V2);
        return meldung;
