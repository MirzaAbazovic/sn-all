...
public class SendWitaSerPowTestAction extends AbstractWitaWorkflowTestAction {

    private final AcceptanceTestDataBuilder acceptanceTestDataBuilder;

    public SendWitaSerPowTestAction(AcceptanceTestDataBuilder acceptanceTestDataBuilder) {
        super("sendSerPow");
    ...
    public void doExecute(AcceptanceTestWorkflow workflow, TestContext testContext) {
        try {
            workflow.sendSerPow(acceptanceTestDataBuilder, Auftragsposition.ProduktBezeichner.HVT_2H);
            testContext.getVariables().put(VariableNames.TEST_DATA, workflow.getCreatedData());
        }
        catch (Exception e) {
            throw new CitrusRuntimeException("Failed to send Wita SerPow", e);
        }
    }
