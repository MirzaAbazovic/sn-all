...
public class TalLae_VZM_PV_KfTest extends AbstractWitaAcceptanceTest {
    ...
    public void talLae01() throws Exception {
        ...
        useCase(WitaAcceptanceUseCase.TAL_AENDERUNG_LAE_VZMPV_01, WitaCdmVersion.V1);

        AcceptanceTestWorkflow workflowLae = workflow().get();
        workflow().select(workflowLae)
                .send(
                        workflowLae.createData(
                                getLaeBuilder(TAL_AENDERUNG_LAE_VZMPV_01, Produkt.AK_CONNECT, baseOrder.auftrag.getAuftragId())),
    ...
    private CreatedData createBaseOrder(WitaAcceptanceUseCase useCase, WitaSimulatorTestUser userName) throws Exception {
        useCase(useCase, WitaCdmVersion.V1);
