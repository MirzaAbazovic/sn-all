...
public class StandortDataBuilder {
    ...
    public StandortData build() throws Exception {
        HVTGruppe betriebsraumGruppe = createHvtGruppe("betriebsraum-");
        HVTStandort betriebsraumStandort = createHvtStandort(betriebsraumGruppe.getId());

        HVTGruppe funktionsraumGruppe = createHvtGruppe("funktionsraum-");
        HVTStandort funktionsraumStandort = createHvtStandort(funktionsraumGruppe.getId());
        ...
        HWBaugruppe hwBaugruppeOlt = new HWBaugruppeBuilder()
        ...
        hwService.saveHWBaugruppe(hwBaugruppeOlt);
    ...
    private HVTStandort createHvtStandort(Long hvtGruppeId) throws StoreException {
        HVTStandort hvtStandort = new HVTStandortBuilder().get();
        hvtStandort.setHvtGruppeId(hvtGruppeId);
        hvtStandort = hvtService.saveHVTStandort(hvtStandort);
        return hvtStandort;
    }

    private HVTGruppe createHvtGruppe(String ortsteilPrefix) throws StoreException {
        HVTGruppe hvtGruppe = new HVTGruppeBuilder().get();
        hvtGruppe.setOrtsteil(ortsteilPrefix + hvtGruppe.getOrtsteil());
    ...
    private HWDpo createHwMdu(HVTStandort hvtStandort, HVTTechnik hvtTechnik, HWBaugruppenTyp hwBaugruppenTyp, HWOlt hwOlt) throws StoreException, de.augustakom.common.tools.validation.ValidationException {
        HWDpo hwDpo = new HWDpoBuilder().withHWRackOltBuilder(new HWOltBuilder()).build();
        hwDpo.setHvtIdStandort(hvtStandort.getId());
        hwDpo.setOltRackId(hwOlt.getId());
        hwDpo.setOltSlot("01");
        hwDpo.setRackTyp(HWRack.RACK_TYPE_DPO);
        hwDpo.setHwProducer(hvtTechnik.getId());
        hwDpo.setOltGPONId("123");
        hwDpo.setSerialNo(null);
        hwDpo = hwService.saveHWRack(hwDpo);
        ...
        hwBaugruppeMdu.setRackId(hwDpo.getId());
