 ...
 * All rights reserved.
 * -------------------------------------------------------
 * File created: 21.02.2005 12:54:11
 */
...
import de.augustakom.authentication.gui.tree.DbTreeGroup;
...
/**
 * Frame, um ein AKDb-Objekt darzustellen.
 */
public class DbDataFrame extends AKJAbstractInternalFrame {

    private static final Logger LOGGER = Logger.getLogger(DbDataFrame.class);
    ...
    private DbDataPanel dataPanel = null;

    /**
     * Konstruktor fuer das Frame mit Angabe des Models und eines TreeNodes.
     *
     * @param model    Angabe des Modells (vom Typ AKDb), das dargestellt werden soll
     ...
     */
    public DbDataFrame(AKDb model, AKJDefaultMutableTreeNode treeNode) {
        super("de/augustakom/authentication/gui/db/resources/DbDataFrame.xml");
    ...
    protected void createGUI() {
        String title = getTitle() + ((model.getName() != null) ? model.getName() : getSwingFactory().getText("new.db"));
        setTitle(title);
        ...
        dataPanel = new DbDataPanel(model);
    ...
    private void save() {
        try {
            ...
            if (DbId == null) {
                ...
                AKJDefaultMutableTreeNode newParent = treeModel.findNode(DbTreeGroup.class, null, null);
